...
func updatePostById(postID int, p UpdatePostPayload, wg *sync.WaitGroup) {
	defer wg.Done()
...
func main() {
	...
	go updatePostById(postID, UpdatePostPayload{Title: &title}, &wg)
	go updatePostById(postID, UpdatePostPayload{Content: &content}, &wg)
	wg.Wait()
}
